...
int CNFparser(CNF &S, char file[], LITSHEET* ans)
//6. 解析CNF文件
{
    FILE *fp;
    char c; int t;
...
// 调整并初始化ans数组
	if(S.num_clau > SUDOKU_VAR) {
		
		LITSHEET* temp;
		temp = realloc(ans,(S.num_clau + 1)* sizeof(Litsheet) );
		if(!temp){
			printf("ERROR in realloc !\n");
			return 0;
		}else{
			ans = temp;
			for(int i = 0;i <= S.num_clau; i++){
				ans[i].ans = 0;
				ans[i].pos = NULL;
				ans[i].nag = NULL;
			}
...
void showLitsheet(LITSHEET* ans)
//9
{
	int cnt = 0;
	for(int i = 1;i <= SUDOKU_VAR; i++){
...
//10显示某一子句 
{
	...
	printf("the clause has %d literals \n",node->num);
	while(lit){
		printf("%d ",lit->value);
		lit = lit ->next;
...
//11. 判断是否为单子句
{
    ...
    return clause->num == 1; // 如果子句中只有一个文字，则为单子句
}
...
int deleteOneClause(CLAUSE *node, stack <INFO_C> &op_clau, CNF &S) 
//15.将指针放进回收栈，返回单子句中变量的值 
{
	if(!node)	return 0;
	int value = node->lit->value; 
	INFO_C opt;
	...
	if(pre == NULL){						//	first clau
		...
		if(S.num_clau == 1) S.tail = S.head;				//	只剩这一个了,此时S为NULL 
	}else{
		opt.self = node;
		opt.pre = pre;		
...
int deleteClause(stack <INFO_C> &op_clau, CNF &S, SHEET ans, int backtrace[]/*,FILE *test*/)
//16. 删除所有单子句，并对CNF进行化简；记录操作次数,将单子句中变量值记录 
{
	CLAUSE *node = existUnitClause(S.head);
	int cnt = 0, value = 0, trueclau = 0, *num_TrueClaus = &cnt;
	
	while(node != NULL){
		// 找到并删除单子句 
		
		node->isTrue = true; 
		value = deleteOneClause(node, op_clau, S);
...
//17.	删除子句中的文字，同时也会删除真子句并记录真子句次数 
{
	...
	CLAUSE *node = S.head;
	int cnt = 0, index = abs(value);		//	直接删除包含value的子句，删除包含-value的文字 
	
	if(value > 0){
		INFO_L * pos = ans[index].pos;
		while(pos){
			
			pos->clause.isTrue = true;
			pos->clause.num --;
			deleteOneClause(node, op_clau,S);
			cnt ++;
			
			pos = pos->next;
		}
		
		INFO_L *nag = ans[index].nag;
...
//21.
{
	int backtrace[MAX_BACK] = {0}; 	
 	int cntCLAU = 0, *cnt_clau = &cntCLAU;
 	int chosenlit = 0;
	...
	cntCLAU += deleteClause(op_clau,S, ans, backtrace/*,test*/);
 	...
 	// 1.成功解出 
	if(!S || !S.head || S.num_clau == 0)	return true;
	// 2.出现矛盾 
	else if(existEmptyClause(S.head)) {

		restore_cl(cntCLAU,op_clau, S,ans);
		restore_lit(cntLIT,op_lit, S);

		return false;		
	}	
	// 3.选取一个变量赋值 
	int chosen = choose_lit(S.head,ans);
...
void check(int ans[], int cnt)
//22.	输出直观结果 
{
	for(int i =1; i <= cnt; i++){
		printf("var %d :",i);
		if(ans[i] == 1)	printf(" TRUE\n");
		else if(ans[i] == 2)	printf(" FALSE\n");
		else printf("Whatever\n"); 
	}
	printf("\n");
}

void check(FILE *fp,int ans[],int cnt)
//23.
{
	for(int i =1; i <= cnt; i++){
		fprintf(fp,"var %d :",i);
		if(ans[i] == 1)	fprintf(fp,"%s"," TRUE\n");
		else if(ans[i] == 2)	fprintf(fp,"%s"," FALSE\n");
...
//	5.达到目标空洞数时输出数独初盘 
//void recoverCNF(CNF &S,stack <INFO_L> &op_lit, stack <INFO_C> &op_clau,int ans[])
////1.	将删去的节点全部复原
//{ 
//	while(!op_clau.empty()){
...
//}
//
//bool DFS_board(/*FILE *test,*/int row, int col, int cnt, stack <INFO_L> &op_lit, stack <INFO_C> &op_clau, CNF &S, int board[N][N],int ans[])
////9.
//{
...
//			
//			if(DPLL(op_lit,op_clau, S, ans/*,test*/)){
//				flag =  DFS_board(/*test,*/i + (j + 1)/9, (j + 1)%9, cnt + 1,op_lit,op_clau,S,board,ans);
//				if(flag)	return true;
//			}
//			//	DPLL或DFS失败
//			recoverCNF(S,op_lit, op_clau,ans);
//			deleteS_tail(S, base);
//		}
//		
//		board[i][j] = 0; 
...
//
//
//bool generate(/*FILE *test,*/stack <INFO_L> &op_lit, stack <INFO_C> &op_clau, CNF &S, int board[N][N], int ans[])
////10.	通过拉斯维加斯算法生成终盘
//{
...
//		
//		btemp = DPLL(op_lit, op_clau, S, ans/*,test*/);
//		printf("bool :%d\n",btemp);
//		recoverCNF(S, op_lit, op_clau, ans);
//		printf("bool :%d\n",btemp);
//		if(btemp){
